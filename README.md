...
- AI-generated DSA problems through Gemini structured JSON generation, validated by the API and saved into the shared problem catalog with an `isAIGenerated` flag.
...
AI problem generation uses the Gemini API when `GEMINI_API_KEY` is configured. The free-tier/demo flow validates JSON shape, required fields, and basic test-case structure, but it does not prove every generated test case with a paid judge or human review before publishing it to the shared AI problem catalog.
