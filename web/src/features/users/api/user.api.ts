...
export const userApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getDashboardAnalytics: builder.query<DashboardAnalyticsResponse, { range?: DashboardAnalyticsRange } | void>({
      query: (arg) => `/user/dashboardAnalytics?range=${arg?.range || '30d'}`,
      providesTags: ['DashboardAnalytics'],
    }),
    trackProfileView: builder.mutation<TrackProfileViewResponse, string>({
      query: (id) => ({
        url: `/user/trackProfileView/${id}`,
        method: 'POST',
      }),
    }),
