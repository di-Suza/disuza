...
import { BadRequestError, AppError } from '../../shared/errors/index.js';
import { type ProblemLanguage, type ProblemTestCase } from './problem.model.js';

const languageIdMap: Record<ProblemLanguage, number> = {
  javascript: 63,
  python: 71,
  cpp: 54,
};
...
function buildExecutableSource(input: { sourceCode: string; language: ProblemLanguage; testInput: string }) {
...
class CodeExecutionService {
  ...
  private async createSubmission(input: { sourceCode: string; language: ProblemLanguage; expectedOutput: string }) {
  ...
  private async runSubmission(input: { sourceCode: string; language: ProblemLanguage; expectedOutput: string }) {
...
export { CodeExecutionService, type ExecutionResult, type TestCaseResult };
